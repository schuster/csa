...
(define (make-sender)
  ...
  (define sender (Sender receiver-channel 80 status-channel))
  (values sender receiver-channel status-channel))

(define (goto-Connected from-receiver status-channel)
  (async-channel-put from-receiver 'SynAck)
  (check-unicast-match status-channel (list 'Connected _)))

(test-case
 "Initial connection time-out"
 (match-define-values (_ _ status-channel) (make-sender))
 (sleep 3)
...
(test-case
 ...
 (match-define-values (sender _ status-channel) (make-sender))
 (async-channel-put sender 'SynAck)
 (check-unicast-match status-channel (list 'Connected _))
...
(test-case
 ...
 (match-define-values (sender to-receiver status-channel) (make-sender))
 (goto-Connected sender status-channel)
 (define req-response (make-async-channel))
 (async-channel-put sender (list 'Write req-response))
...
(test-case
 ...
 (define to-net (make-async-channel))
 (define to-app (make-async-channel))
 (define receiver (Receiver to-net to-app))
 (async-channel-put receiver (list 'Write 80 'Seq0 'Unit))
...
(test-case
 ...
 (define receiver (Receiver to-net to-app))
 (check-unicast to-net 'SynAck)
 (async-channel-put receiver (list 'Write 80 'Seq1 'Unit))
 (check-unicast to-net 'Ack1 #:timeout 1.1)
 (check-unicast to-net 'Ack1 #:timeout 1.1))
...
;; "closed" back, get "write failed" after a new write request
...
(test-case
 ...
 (define manager (Manager app-channel (make-async-channel)))
 (async-channel-put manager
                    (list 'Connect manager 80 connection-status-channel))
 (define sender
   (check-unicast-match connection-status-channel (list 'Connected sender) #:result sender))
 (async-channel-put sender (list 'Write send-response-channel))
 (check-unicast send-response-channel 'Queued)
 (sleep .1)
 (async-channel-put sender 'Close)
